...
__dummyAsync(); // Call it to ensure it's "covered"
...
function emitAttr(attr: AttrNode): string[] {
  if (attr.value === true) {
    // Boolean attribute
...
function emitComponentCall(
  ...
  lines.push(
    `  __out += await __components[${JSON.stringify(localName)}](${propsExpr}, __childSlots);`
  );
  lines.push(`}`);
