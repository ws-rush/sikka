...
| `.mdx`, framework components (`.tsx`, `.vue`, …) | Frontmatter imports accept **only `.astro` Components**; everything else arrives via props                        |
...
| `include('partial')`                  | `import Partial from './Partial.astro'` + `<Partial />`                                                                     |
| `data` option                         | `props` → `Astro.props`                                                                                                     |
| Layouts via `include` + header/footer | A `Layout.astro` Component wrapping children via `<slot />`                                                                 |

The mental shift: Eta interpolates inside a single file; Sikka composes Components. Convert each partial/layout into a `.astro` Component with `Astro.props` for its data and `<slot>` for its content — the [examples app](../examples/README.md) shows the full pattern (Layout, Header, data-driven pages).
