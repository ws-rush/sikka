...
8.  **Verification Flow**: Always run the mandatory validation pipeline (`format`, `lint`, `fallow`, `typecheck`, `test`, `test:coverage`) after any code change. If all pass, update documentation (`README.md`, `AGENTS.md`) to reflect current project state.
